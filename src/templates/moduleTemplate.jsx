...
export default function ModuleTemplate({ data, pageContext }) {
  const { slug } = pageContext;

  const { site, markdownRemark, curriculum } = data;
  const {
  ...
  const {
    frontmatter: { curriculum: curriculumTitle, description, modules },
  } = curriculum;
      ...
      <div className="parent__container">
        <div className="curriculum__container">
          <div className="module__table">
            ...
            <section className="module__table-list">
              <ol>
                {modules?.map((m) => (
                  <li>
                    <a href={`/curriculum/${curriculumTitle}/${m.slug}`}>
                      {m.title}
                    </a>
                  </li>
                ))}
          ...
          <div className="module__left">
            ...
            <div className="module__nav-container">
              {previousModule >= 0 ? (
                <Button
                  link
                  to={`/curriculum/${curriculumTitle}/${modules[previousModule].slug}`}
                  text="< Previous"
                />
              ) : (
                <div />
              )}
              {nextModule < modules.length ? (
                <Button
                  to={`/curriculum/${curriculumTitle}/${modules[nextModule].slug}`}
                  link
                  text="Next >"
                />
              ) : (
...
export const pageQuery = graphql`
  query ($slug: String!, $curriculum: String!) {
    ...
    curriculum: markdownRemark(
      frontmatter: {
        template: { eq: "CurriculumTemplate" }
